...
_TOOLS = [
    {
        ...
        "description": (
            ...
            "Returns the prompt, tool calls, model response, token usage, and latency."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "get_session",
        "description": (
            "Retrieve all LLM calls in an agent run, ordered by time. "
            "Use this to reconstruct the full decision chain for a session."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
async def _handle_tool_call(id_: Any, params: dict, request: Request) -> JSONResponse:
    ...
    if name == "explain":
        ...
        return JSONResponse(_ok(id_, _text_content(json.dumps(record, indent=2))))
    ...
    if name == "get_session":
        ...
        return JSONResponse(_ok(id_, _text_content(json.dumps(records, indent=2))))
